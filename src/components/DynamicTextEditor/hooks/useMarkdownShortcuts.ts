...
const useMarkdownShortcuts = (quill: Quill | null, isModifyingText?: MutableRefObject<boolean>) => {
  ...
  const handleAsteriskInput = useCallback(
    (event: KeyboardEvent) => {
      ...
      const hasMultipleAsterisks = (cursorPosition > 1 && text[cursorPosition - 1] === "*" && text[cursorPosition - 2] === "*") || (cursorPosition < text.length - 1 && text[cursorPosition] === "*" && text[cursorPosition + 1] === "*");
  ...
  const checkForDeletedAsterisks = useCallback(
    (currentText: string) => {
      ...
      for (let i = 0; i < currentText.length; i++) {
        ...
        if (currentText[i] !== lastProcessedText.current[i]) {
          ...
          if (lastProcessedText.current[i] === "*") {
            ...
            const affectedRegion = formattedRegions.current.find((region) => region.start === i || region.end === i);

            if (affectedRegion) {
              // Remove formatting for this region
              console.debug("Removing formatting for region", affectedRegion);
              quill.formatText(affectedRegion.start + 1, affectedRegion.end - affectedRegion.start - 1, { bold: false });
            }
          }
          break;
        }
